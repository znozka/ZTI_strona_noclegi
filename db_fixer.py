...
# # Lista wszystkich tabel w bazie danych w kolejności do pobrania backupu
# TABELE_DO_BACKUPU = [
#     "uzytkownicy",
#     "noclegi",
#     "udogodnienia",
#     "noclegi_udogodnienia",
#     "zdjecia_noclegu",
...
# def wykonaj_pelny_backup(cursor):
#     """Pobiera każdą tabelę z bazy danych i zapisuje do osobnego pliku CSV jako backup."""
#     print("=== ROZPOCZYNANIE PEŁNEGO BACKUPU BAZY DANYCH ===")
    
#     for tabela in TABELE_DO_BACKUPU:
#         nazwa_pliku = f"backup_{tabela}.csv"
#         try:
#             # Dynamiczne zapytanie pobierające całą zawartość tabeli
#             cursor.execute(f"SELECT * FROM {tabela}")
            
#             # Wyciągnięcie nagłówków kolumn
#             kolumny = [column[0] for column in cursor.description]
            
#             # Zapis do pliku CSV (średnik jako separator z uwagi na przecinki w opisach/nazwach)
#             with open(nazwa_pliku, mode="w", newline="", encoding="utf-8") as f:
#                 writer = csv.writer(f, delimiter=";")
#                 writer.writerow(kolumny)  # Zapis nagłówka
                
#                 licznik_wierszy = 0
#                 while True:
#                     wiersze = cursor.fetchmany(100)
#                     if not wiersze:
#                         break
#                     for wiersz in wiersze:
#                         writer.writerow(list(wiersz))
#                         licznik_wierszy += 1
                        
#             print(f" -> Tabela '{tabela}': Zapisano {licznik_wierszy} wierszy do pliku {nazwa_pliku}")
            
#         except Exception as e:
#             print(f" [!] Błąd podczas tworzenia backupu dla tabeli {tabela}: {e}")
#             raise e
            
#     print("=== PEŁNY BACKUP ZAKOŃCZONY SUKCESEM ===\n")


# def usun_noclegi(cursor, min_id, max_id):
#     """Wykonuje usunięcie noclegów. Dzięki ON DELETE CASCADE silnik bazy

#     samoczynnie wyczyści powiązane tabele zależne.
#     """
#     print(f"Modyfikacja bazy: Usuwanie noclegów o ID od {min_id} do {max_id}...")
    
#     # Skoro potwierdzono, że rezerwacje, opinie i ulubione nie mają tych obiektów,
#     # możemy uderzyć bezpośrednio w główną tabelę.
#     cursor.execute(
#         "DELETE FROM noclegi WHERE id_noclegu BETWEEN ? AND ?", (min_id, max_id)
#     )
    
#     ile_usunieto = cursor.rowcount
#     print(f" -> Sukces: Usunięto {ile_usunieto} rekordów bezpośrednio z tabeli 'noclegi'.")


# def main():
#     conn = None
#     try:
#         # Utworzenie bezpiecznego połączenia
#         conn = pyodbc.connect(CONNECTION_STRING)
#         cursor = conn.cursor()

#         # KROK 1: Wykonanie pełnego backupu do plików CSV (zabezpieczenie przed usunięciem)
#         wykonaj_pelny_backup(cursor)

#         # KROK 2: Bezpieczne usunięcie rekordów
#         usun_noclegi(cursor, MIN_ID, MAX_ID)

#         # KROK 3: Zatwierdzenie transakcji, jeśli wszystko przebiegło bezbłędnie
#         conn.commit()
#         print("\n[INFO] Wszystkie operacje powiodły się. Transakcja została zatwierdzona w bazie.")

#     except Exception as e:
#         # W przypadku błędu na dowolnym etapie (nawet podczas tworzenia plików backupu)
#         # baza danych nie zostanie zmodyfikowana.
#         if conn:
#             conn.rollback()
#             print("\n[ALERT] Wystąpił błąd krytyczny! Wycofano wszelkie zmiany w bazie danych (Rollback).")
#         print(f"Szczegóły błędu technicznego: {e}")

#     finally:
#         # Zamknięcie sesji połączenia
#         if conn:
#             conn.close()
