...
if selected_id is None:
    ...
    try:
        ...
        if df_nocleg.empty:
            ...
            # MODYFIKACJA 1: Zmiana sortowania opinii na malejąco po ocenie
            query_opinie = """
                SELECT o.ocena, o.komentarz, u.imie
                FROM opinie o
                JOIN uzytkownicy u ON o.id_turysty = u.id_uzytkownika
            ...
            with col_side:
                ...
                with opinie_box:
                    ...
                    if not df_opinie.empty:
                        for _, opinia in df_opinie.iterrows():
                            # MODYFIKACJA 2: Zamiana imienia turysty na gwiazdkę (*)
                            st.markdown(f"**<span style='color: #EAB308;'>{opinia['ocena']}/5</span>** (*)", unsafe_allow_html=True)
                            st.caption(opinia['komentarz'])
                            st.markdown("<br>", unsafe_allow_html=True)
                    else:
                        st.markdown("**5/5**")
