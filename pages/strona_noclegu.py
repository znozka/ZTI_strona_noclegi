    ...
    try:
        ...
        if df_nocleg.empty:
            ...
            query_opinie = """
                ...
                ORDER BY 
                    CASE WHEN o.komentarz IS NULL OR o.komentarz = '' THEN 1 ELSE 0 END ASC,
                    o.ocena DESC
            """
            ...
            with col_main:
                # Przygotowanie placeholderów na zdjęcia
            ...
            with col_side:
                ...
                with opinie_box:
                    ...
                    if "pokaz_wszystkie_opinie" not in st.session_state:
                        st.session_state.pokaz_wszystkie_opinie = False

                    # ===== FILTER OPINIONS =====
                    df_opinie_z_komentarzem = df_opinie[
                        df_opinie['komentarz'].notna() &
                        (df_opinie['komentarz'].str.strip() != "")
                    ].copy()
                    ...
                    # ===== FULL LIST VIEW =====
                        ...
                        with opinie_scroll:
                            for _, opinia in df_opinie.iterrows():
                                ...
                                # Sprawdzenie czy komentarz nie jest NaN lub pustym ciągiem znaków
                                if komentarz and str(komentarz).strip() and str(komentarz) != "NaN" and not pd.isna(komentarz):
                                    st.caption(komentarz)

                                st.markdown("<hr style='margin:6px 0; border:0; border-top:1px solid #eee;'>",
                                            unsafe_allow_html=True)

                        if st.button("Ukryj pełną listę", key="ukryj_opinie_btn", use_container_width=True):
                            st.session_state.pokaz_wszystkie_opinie = False
                            st.rerun()
                    ...
                    # ===== CAROUSEL VIEW =====
                        ...
                        if df_opinie_z_komentarzem.empty:
                            ...
                            # ===== CAROUSEL =====
                                ...
                                procent_wejscia = (czas_przejscia / laczny_czas) * 100
                                procent_widocznosci = ((czas_przejscia + czas_wyswietlania) / laczny_czas) * 100
                                procent_wyjscia = ((czas_przejscia * 2 + czas_wyswietlania) / laczny_czas) * 100
                                ...
                                .carousel-slide {{
                                    ...
                                    animation: slideAnimation {laczny_czas}s infinite ease-in-out;
                                }}

                                @keyframes slideAnimation {{
                                    0% {{ opacity:0; transform:translateX(100%); }}
                                    {procent_wejscia}% {{ opacity:1; transform:translateX(0%); }}
                                    {procent_widocznosci}% {{ opacity:1; transform:translateX(0%); }}
                                    {procent_wyjscia}% {{ opacity:0; transform:translateX(-100%); }}
